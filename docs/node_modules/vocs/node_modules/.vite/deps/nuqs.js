  ...
  function loadSearchParams(input, { strict = false } = {}) {
    ...
    for (const [key, parser] of Object.entries(parsers)) {
      ...
      try {
        ...
        if (strict) throw new Error(`[nuqs] Error while parsing query \`${query}\` for key \`${key}\`: ${error2}`);
        parsedValue = null;
      }
  ...
  return { "~standard": {
    ...
    validate(input) {
      try {
        ...
        return { issues: [{ message: error2 instanceof Error ? error2.message : String(error2) }] };
      }
    }
  } };
}
...
function useQueryStates(keyMap, options = {}) {
  ...
  const update = (0, import_react.useCallback)((stateUpdater, callOptions = {}) => {
    ...
    for (let [stateKey, value] of Object.entries(newState)) {
      ...
      if (callOptions?.limitUrlUpdates?.method === "debounce" || limitUrlUpdates?.method === "debounce" || parser.limitUrlUpdates?.method === "debounce") {
        if (update2.options.shallow === true) console.warn(error(422));
        ...
        const debouncedPromise = debounceController.push(update2, timeMs, adapter, processUrlSearchParams);
        ...
        globalThrottleQueue.push(update2, timeMs);
        doFlush = true;
      }
    }
  ...
  return [(0, import_react.useMemo)(() => applyDefaultValues(internalState, defaultValues), [internalState, defaultValues]), update];
}
...
function useQueryState(key, options = {}) {
  ...
  return [state, (0, import_react.useCallback)((stateUpdater, callOptions = {}) => setState((old) => ({ [key]: typeof stateUpdater === "function" ? stateUpdater(old[key]) : stateUpdater }), callOptions), [key, setState])];
}
export {
  createLoader,
  createMultiParser,
